...
Feature: 가용 사이트 검색
  ...
  # [정상 케이스] 기본 검색
  Scenario: 예약이 없는 기간으로 검색 시 예약 가능한 사이트 목록 반환된다
  ...
  # [정상 케이스] 필터링 및 정보 표시
  Scenario: 사이트 크기 필터링 및 상세 정보 확인이 가능하다
  ...
  Scenario: 기간 내 중간 날짜가 예약된 사이트는 검색에서 제외된다 (연박 불가)
  ...
  # [예외 케이스] 날짜 유효성 검증
  Scenario: 과거 날짜 포함하여 검색할 경우 요청이 거부된다
...
Feature: 예약 취소
  ...
  Scenario: 올바른 예약 확인 코드로 취소 시 재고가 즉시 복구된다
    Given "A-2" 사이트가 해당 예약(ID 100)으로 점유되어 있다
