Feature: 예약 생성
  ...
  Scenario: [실패] 이미 예약된 날짜에 중복 예약을 시도한다
    When 다른 사용자가 'A-101' 사이트의 이미 예약된 날짜를 포함하여 예약을 요청하면
...
Feature: 가용성 확인
  ...
  Scenario: [성공] 사용자가 조회한 기간에 예약 가능한 사이트가 있다
    ...
    Then 조회 결과에 예약 가능한 모든 사이트가 포함되어야 한다.

  Scenario: [실패] 연박 기간 중 일부 날짜가 예약 불가능하다
    When 'C-301' 사이트가 '2025-11-02'에는 이미 예약된 경우, 사용자가 '2025-11-01'부터 '2025-11-03'까지의 가용성을 검색하면
    Then 조회 결과에 'C-301' 사이트가 포함되지 않아야 한다.

  Scenario: [정보] 조회한 기간에 예약 가능한 사이트가 하나도 없다
    When '2025-12-24'부터 '2025-12-25'까지 모든 사이트가 예약 마감된 경우, 사용자가 해당 기간의 가용성을 검색하면
