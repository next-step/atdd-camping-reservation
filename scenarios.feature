Feature: 캠핑장 예약 생성
  ...
  Scenario: 필수 정보 누락으로 예약 실패
    ...
    And 예약하려는 고객의 정보는 다음과 같이 주어진다:
      | customerName | phoneNumber   | siteNumber | startDate  | endDate    |
      |              | 010-1234-5678 | A-1        | 2025-01-15 | 2025-01-17 |
    When 주어진 정보로 예약을 시도한다
    Then 예약이 실패한다
  ...
  Scenario: 30일 제한 규칙을 한참 넘는 위반으로 예약 실패
    ...
    And 예약하려는 고객의 정보는 다음과 같이 주어진다:
      | customerName | phoneNumber   | siteNumber | startDate  | endDate    |
      | 김철수       | 010-1234-5678 | A-1        | 2025-02-15 | 2025-02-17 |
    When 주어진 정보로 예약을 시도한다
    Then 예약이 실패한다
  ...
  Scenario: 30일 제한 규칙을 애매하게 넘는 위반으로 예약 실패
    ...
    And 예약하려는 고객의 정보는 다음과 같이 주어진다:
      | customerName | phoneNumber   | siteNumber | startDate  | endDate    |
      | 김철수       | 010-1234-5678 | A-1        | 2025-01-31 | 2025-02-05 |
    When 주어진 정보로 예약을 시도한다
    Then 예약이 실패한다
    And "오늘로부터 30일 이내로 예약할 수 있습니다" 오류 메시지가 반환된다
  ...
  Scenario: 종료일이 시작일보다 이전인 경우 예약 실패
    ...
    And 예약하려는 고객의 정보는 다음과 같이 주어진다:
      | customerName | phoneNumber   | siteNumber | startDate  | endDate    |
      | 김철수       | 010-1234-5678 | A-1        | 2025-01-16 | 2025-01-15 |
    When 주어진 정보로 예약을 시도한다
    Then 예약이 실패한다
  ...
  Scenario: 예약 일자가 애매하게 겹치면 예약 실패
    ...
    And 예약하려는 고객의 정보는 다음과 같이 주어진다:
      | customerName | phoneNumber   | siteNumber | startDate  | endDate    |
      | 이영희       | 010-9876-5432 | A-1        | 2025-01-17 | 2025-01-18 |
    When 주어진 정보로 예약을 시도한다
    Then 예약이 실패한다
    And "해당 기간에 이미 예약이 존재합니다" 오류 메시지가 반환된다
