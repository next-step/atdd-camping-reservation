Feature: 동시성 제어
  ...
  Scenario: [정상 케이스]
    Given 두 명의 사용자가 동일한 캠핑 사이트와 날짜로 예약을 시도한다
    When 두 예약 요청이 거의 동시에 처리된다
    Then 하나의 예약만 성공하고 다른 하나는 실패한다
...
Feature: 연박 예약
  ...
  Scenario: [정상 케이스]
    Given 사용자가 3박 4일 연박 예약을 시도한다
    When 모든 날짜가 예약 가능하다
    Then 예약이 성공한다

  Scenario: 연박 기간 중 일부 날짜가 이미 예약된 경우
    Given 사용자가 3박 4일 연박 예약을 시도한다
    When 중간 날짜가 이미 예약되어 있다
    Then 예약이 실패한다
...
Feature: 예약 생성
  ...
  Scenario: [정상 케이스]
    Given 사용자가 필수 정보를 모두 입력하고 예약을 시도한다
    When 날짜 검증을 통과하고 동일 사이트와 기간에 중복 예약이 없다
    Then 예약이 성공한다

  Scenario: 필수 정보가 누락된 경우
    Given 사용자가 필수 정보를 누락하고 예약을 시도한다
    When 날짜 검증을 통과한다
    Then 예약이 실패한다

  Scenario: 날짜 검증에 실패한 경우
    Given 사용자가 필수 정보를 모두 입력하고 오늘로부터 30일 이후 시점에 예약을 시도한다
    When 날짜 검증에 실패한다
    Then 예약이 실패한다

  Scenario: 동일 사이트와 기간에 중복 예약이 있는 경우
    Given 사용자가 필수 정보를 모두 입력하고 예약을 시도한다
    When 동일 사이트와 기간에 이미 예약이 존재한다
    Then 예약이 실패한다

  Scenario: 동일 사이트와 기간에 취소된 예약이 있는 경우
    Given 사용자가 필수 정보를 모두 입력하고 예약을 시도한다
    When 동일 사이트와 기간에 취소된 예약이 존재한다
    Then 예약이 성공한다