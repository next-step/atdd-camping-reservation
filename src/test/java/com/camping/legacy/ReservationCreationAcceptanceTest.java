...
class ReservationCreationAcceptanceTest extends AcceptanceTest {
  ...
  void 유효한_정보로_예약요청시_예약이_확정된다() {
    ...
    // then
    예약_결과_검증(응답, HttpStatus.CREATED);
  ...
  void 예약_제한_기간을_초과하여_요청하면_예약이_거부된다() {
    ...
    // then
    예약_결과_검증(응답, HttpStatus.CONFLICT);
  ...
  void 성수기_주말_할증_요금이_자동_계산된다() {
    ...
    var 예약_요청 = aReservationRequest().withStartDate(토요일).withEndDate(일요일);
  ...
  void 다양한_날짜_경계값_예약_테스트(int startDayOffset, int endDayOffset, HttpStatus expectedStatus) {
    ...
    var 예약_요청 = aReservationRequest()
        ...
        .withEndDate(endDayOffset);

    // when
    var 응답 = 예약을_요청한다(예약_요청.build());

    // then
    예약_결과_검증(응답, expectedStatus);
  }
  ...
  void 기존_예약과_기간이_일치하면_거부된다() {
    // given
    var 기존_예약_요청 = aReservationRequest()
        .withStartDate(5)
        .withEndDate(7);
    예약을_요청한다(기존_예약_요청.build());

    // when
    var 신규_예약_요청 = aReservationRequest()
        .withStartDate(5)
        .withEndDate(7);
    var 응답 = 예약을_요청한다(신규_예약_요청.build());
