...
class ReservationAcceptanceTest extends ApiAcceptanceTestBase {
    ...
    void 예외_동일_사이트의_기간이_겹치면_중복_예약이_거부된다() {
        ...
        ExtractableResponse<Response> response = 예약을_생성한다(ReservationRequestFixture.builder()
                .customerName("김철수")
                .startDate(LocalDate.parse(기본_예약_요청.getStartDate()))
                .endDate(LocalDate.parse(기본_예약_요청.getEndDate()))
                .siteNumber("A-1")
                .build());
    ...
    void 예외_동시에_동일_사이트_기간으로_예약_요청이_여러_건_들어와도_하나만_성공해야_한다() throws ExecutionException, InterruptedException {
        ...
        try {
            Callable<Integer> task1 = () -> 예약을_생성한다(ReservationRequestFixture.builder()
                    .customerName("사용자1")
                    .startDate(startDate)
                    .endDate(endDate)
                    .siteNumber("A-1")
                    .build()).statusCode();
            Callable<Integer> task2 = () -> 예약을_생성한다(ReservationRequestFixture.builder()
                    .customerName("사용자2")
                    .startDate(startDate)
                    .endDate(endDate)
                    .siteNumber("A-1")
