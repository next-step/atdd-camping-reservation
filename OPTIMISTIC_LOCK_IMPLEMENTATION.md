...
## 부록: 재시도가 의미 있는 시나리오 (날짜가 겹치지 않는 경우)



질문: 버전 충돌이 일어났다는 것은 이미 다른 사용자가 커밋에 성공했다는 의미입니다. 그렇다면 어차피 예약이 찼을 텐데 재시도가 의미가 있나요?



답변: **네, 매우 의미 있습니다.** 버전 충돌이 "내가 예약하려는 날짜가 예약됨"을 의미하지는 않기 때문입니다. 현재 시스템은 **어떤 날짜의 예약이든 `Campsite`에 예약이 추가되면 버전이 업데이트**됩니다. 따라서, 날짜가 겹치지 않는 예약을 동시에 처리할 때 재시도 메커니즘이 빛을 발합니다.



### 시나리오: 재시도를 통해 예약에 성공하는 경우



-   **상황**: 사용자 A는 8월 5일, 사용자 B는 9월 10일에 각각 **날짜가 겹치지 않는** 예약을 동시에 시도합니다.

-   **초기 상태**: `A-01` 사이트의 버전은 `10`입니다.



1.  **Step 1: 버전 읽기 (사용자 A, B)**

    -   트랜잭션 A(8월 예약): `A-01` 사이트의 버전 `10`을 읽습니다.

    -   트랜잭션 B(9월 예약): 트랜잭션 A가 커밋하기 전에, `A-01` 사이트의 버전 `10`을 읽습니다.



2.  **Step 2: 트랜잭션 A 커밋 (8월 예약 성공)**

    -   사용자 A의 8월 예약이 유효하므로 커밋을 시도합니다.

    -   `UPDATE campsites SET version = 11 WHERE id = <A-01_ID> AND version = 10` 쿼리가 실행되고 성공합니다.

    -   DB의 `A-01` 사이트 버전은 이제 `11`이 됩니다.



3.  **Step 3: 트랜잭션 B 커밋 시도 및 충돌 발생 (9월 예약)**

    -   사용자 B도 9월 예약이 유효하다고 판단하고 커밋을 시도합니다.

    -   `UPDATE campsites SET version = 11 WHERE id = <A-01_ID> AND version = 10` 쿼리가 실행됩니다.

    -   **충돌 발생**: DB의 버전은 `11`이므로 업데이트에 실패하고 `OptimisticLockException`이 발생합니다.



4.  **Step 4: AOP를 통한 재시도 (사용자 B)**

    -   `OptimisticLockRetryAspect`가 예외를 잡아서 `createReservation` 메소드를 다시 실행합니다.



5.  **Step 5: 재시도 성공 (사용자 B)**

    -   재시도된 트랜잭션은 DB에서 `A-01` 사이트의 **최신 버전(`11`)**을 다시 읽어옵니다.

    -   예약 가능 여부를 다시 체크합니다. 사용자 A의 예약은 **8월**이므로, 사용자 B가 예약하려는 **9월**과는 겹치지 않습니다.

    -   `hasConflict`는 `false`가 되고, 예약 로직이 끝까지 성공적으로 진행됩니다.

    -   재시도된 트랜잭션이 커밋되면서 `UPDATE campsites SET version = 12 WHERE id = <A-01_ID> AND version = 11` 쿼리가 실행되고 성공합니다.

    -   **결론**: 사용자 B는 첫 시도에서는 버전 충돌로 실패했지만, 재시도를 통해 자신의 9월 예약을 성공적으로 마칩니다.
